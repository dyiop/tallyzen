...
Two modes, one interface:
  - MOCK (default): answers from seed_books.json. Runs anywhere, zero deps.
  - REAL: set TALLY_HTTP=http://<vm-ip>:9000 and implement the _tally_* helpers
    to POST TDL/XML to Tally. The agent-facing routes below do NOT change.
...
# --- Real TallyPrime hooks (implement when the VM gateway is reachable) ---------
def _tally_post_voucher(payload):
    # POST a TDL/XML <VOUCHER> envelope to TALLY_HTTP. Left as a stub so the mock
    # path is never blocked. See tally/import/generate_tally_xml.py for the shapes.
    raise NotImplementedError("Wire XML POST to TALLY_HTTP here for live writes.")
